#!/usr/bin/env bash
# Copies the conformance corpus from a Pilotage checkout and records where it
...
pilotage_dir="${1:-${PILOTAGE_DIR:-$repo_dir/../Pilotage}}"
if [[ ! -d "$pilotage_dir/.git" ]]; then
    echo "usage: $0 [path-to-Pilotage-checkout]" >&2
    exit 1
fi

upstream_path="$(sed -n 's/^path=//p' "$record")"
revision="$(git -C "$pilotage_dir" rev-parse HEAD)"

git -C "$pilotage_dir" show "HEAD:$upstream_path" > "$fixture"
